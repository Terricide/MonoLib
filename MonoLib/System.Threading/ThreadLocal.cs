...
namespace System.Threading
{
    ...
    public class ThreadLocal<T> : IDisposable
    {
        ...
        private T GetValueSlow()
        {
            ...
            if (id < 0)
            {
                throw new ObjectDisposedException(EnvironmentEx.GetResourceString("ThreadLocal_Disposed"));
            }
            ...
            else
            {
                ...
                if (IsValueCreated)
                {
                    throw new InvalidOperationException(EnvironmentEx.GetResourceString("ThreadLocal_Value_RecursiveCallsToValue"));
                }
            }
        ...
        public IList<T> Values
        {
            get
            {
                if (!m_trackAllValues)
                {
                    throw new InvalidOperationException(EnvironmentEx.GetResourceString("ThreadLocal_ValuesNotAvailable"));
                }

                var list = GetValuesAsList(); // returns null if disposed
                if (list == null) throw new ObjectDisposedException(EnvironmentEx.GetResourceString("ThreadLocal_Disposed"));
                return list;
            }
        }
